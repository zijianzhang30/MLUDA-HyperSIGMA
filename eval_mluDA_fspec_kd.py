...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--device", default="cuda:0" if torch.cuda.is_available() else "cpu"); ap.add_argument("--lambdas", type=float, nargs="*", default=(0.0, 0.05, 0.1, 0.2)); ap.add_argument("--seeds", type=int, nargs="*", default=seeds); ap.add_argument("--cache", type=Path, default=CACHE); ap.add_argument("--root-out", type=Path, default=ROOT_OUT)
    ...
    for lam in args.lambdas:
        directory = args.root_out / f"lambda_{lam:g}"; runs = []
    ...
    (args.root_out / "posthoc_target_summary.json").write_text(json.dumps(summary, indent=2)); print(json.dumps(summary, indent=2))
