...
def main():
  ap=argparse.ArgumentParser(); ap.add_argument('--device',default='cuda:0' if torch.cuda.is_available() else 'cpu'); ap.add_argument('--batch-size',type=int,default=64); ap.add_argument('--output',type=Path,default=OUT); a=ap.parse_args(); dev=torch.device(a.device)
  ...
  m=SSFusionFramework(img_size=33,in_channels=48,patch_size=2,classes=7,model_size='base'); m.load_state_dict(torch.load(CKPT,map_location='cpu')['model'],strict=True); m.to(dev).eval(); [p.requires_grad_(False) for p in m.parameters()]
  sf,tf=extract(m,src,sc,dev,a.batch_size),extract(m,tgt,tc,dev,a.batch_size); assert np.isfinite(sf).all() and np.isfinite(tf).all(); a.output.parent.mkdir(parents=True,exist_ok=True); np.savez(a.output,source_centers=sc,target_centers=tc,source_fspat=sf,target_fspat=tf); print('saved',a.output,sf.shape,tf.shape)
